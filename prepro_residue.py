...
def main():
    ...
    vgg_model = './ResNet-152-model.caffemodel'
    vgg_deploy = './ResNet-152-deploy.prototxt'
    ...
    train_dataset_last = pd.concat([train_dataset, val_dataset[0:train_cutoff]])
    print train_dataset_last
    save_pickle(train_dataset_last, 'data/train/train.annotations.pkl')
    save_pickle(val_dataset[train_cutoff:val_cutoff], 'data/val/val.annotations.pkl')
    save_pickle(val_dataset[val_cutoff:].reset_index(drop=True), 'data/test/test.annotations.pkl')
        ...
        if 1:
          for split in ['train', 'val', 'test']:
            ...
            for start, end in zip(range(0, n_examples, batch_size),
                ...
                image_batch = np.array(map(lambda x: crop_image(x, target_width=224, target_height=224), image_batch_file))

                #feats = sess.run(vggnet.features, feed_dict={vggnet.images: image_batch})
                caffe_in = np.zeros(np.array(image_batch.shape)[[0,3,1,2]], dtype=np.float32)
                for idx, in_ in enumerate(image_batch):
                    caffe_in[idx] = cnn.transformer.preprocess('data', in_)

                out = cnn.net.forward_all(blobs=[layers], **{'data':caffe_in})
                feats = out[layers]
                feats = np.transpose(feats, (0, 2, 3, 1))
                feats = np.reshape(feats, (feats.shape[0], 49, 2048))
                save_path = './data_residue_single/%s/%s_%d.features.hkl' % (split, split, start)
            # use hickle to save huge feature vectors
                hickle.dump(feats, save_path)
