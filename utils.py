...
def sample_coco_minibatch(data, batch_size):
    ...
    test_detect_batch = np.empty((batch_size, 20, 4096))
    m = 0
    for j in mask:
        test_detect_single = hickle.load('./data_residue_detect/test/'+ 'test_' + str(j) + '.features.hkl')
        test_detect_batch[m,:] = test_detect_single 
    features = data['features'][mask]
    file_names = data['file_names'][mask]
    return features, test_detect_batch, file_names
