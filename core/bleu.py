...
def evaluate_part(candidate = '', split='val', idx = 1,  get_scores=True):
  
    reference_path = os.path.join('./data/', "%s/%s.references.pkl" %(split, split))
    cand = candidate
    ...
    # compute bleu score
    final_scores = score(ref_used, hypo)
